...
def main():
    parser = argparse.ArgumentParser(description='阶段2: ResNet-18 + 传统数据增强')
    ...
    parser.add_argument('--pretrained', action='store_true',
                        help='使用预训练权重')
    parser.add_argument('--target_acc', type=float, default=82.0,
                        help='目标准确率 (%)')
    ...
    config = {
        ...
        'device': str(device),
        'augmentation': 'Traditional (flip, rotation, crop, affine)'
    }
    ...
    model = ResNetClassifier(num_classes=3, pretrained=args.pretrained)
    ...
    print(f"差距: {test_acc - args.target_acc:+.2f}%")
    
    print(f"\n各类别准确率:")
    for name, acc in test_class_accs.items():
        print(f"  {name}: {acc:.2f}%")
    
    # 保存结果
    results = {
        'experiment': 'stage2_traditional_augmentation',
        'target_accuracy': args.target_acc,
        'test_accuracy': test_acc,
        'val_accuracy': best_acc,
        'difference_vs_target': test_acc - args.target_acc,
        'epochs_trained': epoch,
        'class_accuracy': test_class_accs,
        'config': config,
        'history': history
    }
    ...
    if test_acc >= args.target_acc:
        print(f"✅ 达到目标！测试准确率 {test_acc:.2f}% ≥ 目标 {args.target_acc}%")
    else:
        print(f"⚠️ 未达到目标。测试准确率 {test_acc:.2f}% < 目标 {args.target_acc}%")
        print(f"   差距: {args.target_acc - test_acc:.2f}%")
    
    return test_acc >= args.target_acc
