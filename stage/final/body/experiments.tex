...
为进一步直观展示最优模型的分类性能，图\ref{fig:confusion_matrix}给出了Stage 5模型在测试集（117张）上的混淆矩阵，图\ref{fig:roc_curves}则展示了三类别的ROC曲线及AUC指标。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.7\textwidth]{images/confusion_matrix.png}
    \caption{Stage 5最优模型测试集混淆矩阵（117张）}
    \label{fig:confusion_matrix}
\end{figure}

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.75\textwidth]{images/roc_curves.png}
    \caption{Stage 5最优模型三类别ROC曲线（宏平均AUC=99.63\%）}
    \label{fig:roc_curves}
\end{figure}

混淆矩阵显示绝大多数样本位于对角线上，少数误判集中发生在良性↔恶性之间，这与超声图像中部分良性肿瘤与恶性肿瘤在形态上存在一定相似性的临床认知一致。ROC曲线则表明三类均接近理想的左上角，AUC值达到0.99以上，充分说明模型具有极强的类别判别能力。
...
图\ref{fig:metrics_comparison}从准确率、精确率、召回率、F1分数等多个维度，可视化对比了五个阶段的关键指标演进，清晰地展现出每个优化环节带来的增益。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.92\textwidth]{images/metrics_comparison.png}
    \caption{五阶段递进实验关键指标对比}
...
对于正确分类的样本，Grad-CAM热力图应当高亮显示肿瘤病灶所在的区域。如图\ref{fig:gradcam_example}所示，对于一张被正确预测为"恶性"的超声图像，Grad-CAM热力图的暖色区域（红色/橙色）高度集中地覆盖了图像中的不规则低回声区域——这正是放射科医生诊断恶性病变时所依据的关键形态特征。类似地，对于良性肿瘤图像，热力图聚焦于边界清晰的圆形或椭圆形回声区域；对于正常组织图像，热力图呈现较为均匀分散的状态，没有异常集中的热点。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.85\textwidth]{images/gradcam_examples.png}
    \caption{Grad-CAM可解释性热力图示例：三行自上而下分别对应良性、恶性、正常类别；三列自左至右分别为原始图像、Grad-CAM热力图、叠加可视化}
...
\subsection{Web 诊断系统演示}

为将上述训练好的模型转化为可供医生与研究人员实际使用的工具，本课题在 Stage 5 最优模型的基础上实现了一套基于 Web 的诊断系统。系统采用前后端分离架构：后端为 Flask REST API（负责模型推理、Grad-CAM 生成与历史记录持久化），前端为 React + TypeScript 单页应用（负责文件上传、结果可视化与交互）。系统支持单张与批量两种分析模式，并提供完整的历史查询功能。

图\ref{fig:ui_single}展示了单张分析模式下系统的完整工作流：用户拖拽上传一张乳腺超声图像后，点击"开始分析"按钮即可在约 100--200 毫秒内获得诊断结论、各类别概率分布以及 Grad-CAM 热力图。图中示例图被模型正确判定为"恶性"，置信度 95.16\%，热力图的暖色区域准确聚焦在图像中央的低回声病灶上。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.82\textwidth]{images/ui_single_result.png}
    \caption{单张分析模式：诊断结论、概率分布与 Grad-CAM 热力图可视化}
    \label{fig:ui_single}
\end{figure}

图\ref{fig:ui_batch}展示了批量分析模式：用户可一次性上传至多 10 张超声图像，系统并行推理并给出汇总的类别分布与恶性比例，每条记录可单独展开查看详细概率。该模式便于快速分析一次随访产生的多张影像或小批量筛查数据。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.82\textwidth]{images/ui_batch_result.png}
    \caption{批量分析模式：分析摘要与逐条诊断记录}
    \label{fig:ui_batch}
\end{figure}

此外，系统使用 SQLite 在本地持久化每一次诊断记录。图\ref{fig:ui_history}展示了历史记录页面，用户可按时间浏览、筛选、删除历次诊断结果，并查看各类别累计次数与平均推理耗时等统计信息。

\begin{figure}[htb]
    \centering
    \includegraphics[width=0.82\textwidth]{images/ui_history.png}
    \caption{历史记录页：统计卡片、记录列表与批量管理操作}
    \label{fig:ui_history}
\end{figure}

整套系统通过多阶段 Docker 镜像打包（Node 构建层 + Python 运行层），可一条 \texttt{docker run} 命令完成部署，为后续在本地工作站或院内服务器上的落地使用提供了便利。
